...
# Disclaimer
В апреле и мае 2018 у меня было время заниматься проектом, потом
 свалились всякие несчастья и проект пришлось отложить.
 
В сентябре 2019 я вспомнил про проект и решил максимально быстро
 довести его до сколько нибудь годного состояния.
 
По идее значения характеристик предметов (позиций каталога) должны
 храниться в соответствующем виде, строки в строках, числа в числах, но
 учитывая аврал в разработке, было реализовано хранение только в строках
 
Вообще много было идей, но жизнь показала, что нет возможности
 реализовать их все сразу, поэтому в коде есть рудименты, не удивляйтесь
 , так было задумано, и не реализовано, теперь выпилить не когда.

Поэтому при поиске по диапазону, сам диапазон значений вычисляется как 
 для строковых значений, а не как для чисел, это надо иметь в виду при
 знакомстве с функционалом.
 
Допилить до "правильного" хранения не долго, всего два дня, но
 когда они у меня будут ?
 
Для демонстрации работы с каталогом разработан
 [тест](/tests/integration/BusinessProcessTest.php) смотрите комментарии в
 коде.
 
Сейчас реализована работа с каталогом только через представление,
 работа с каталогом через материализованное представление и таблицу не
 написана, там работы на пару часов, надо написать свою реализацию для 
 [Источника](/src/AllThings/PrimitiveObtainment/Source.php)
